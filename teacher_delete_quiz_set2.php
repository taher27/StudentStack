...
<html>
    <head>
      ...
      if(mysqli_num_rows($sql)<1)
      {
          $testid = $_REQUEST['testid'];
          echo "<script>";
                                echo "var x=window.confirm('This set is EMPTY');
                                        if (x)
                                        {
                                            window.location='teacher_delete_quiz_set.php?testid=$testid';
                                        }
                                        else
                                        {
                                            window.location='teacher_delete_quiz_set.php?testid=$testid';
      ...
      else
      {
          ...
          {
              echo "<tr><center><td>$srno</td><td>$row[2]</td><td>$row[3]</td><td>$row[4]</td><td>$row[5]</td><td>$row[6]</td><td>$row[7]</td><td><a class='buttonlink' href='teacher_delete_quiz_set3.php?queid=$row[0]&setid=$row[1]&testid=$testid'>DELETE</a></td></center></tr>";
              $srno++;
